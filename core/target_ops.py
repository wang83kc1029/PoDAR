...
def create_target_np(all_anchors,
    ...
    if len(gt_boxes) > 0 and anchors.shape[0] > 0:
        # Compute distances between the anchors and the gt boxes
        anchor_by_gt_dist = similarity_fn(anchors, gt_boxes) # [N_anchor, N_gt]        
        # Map from anchor to gt box that has smallest distance
        anchor_to_gt_argmin = anchor_by_gt_dist.argmin(axis=1) # [N_anchor]
        # For each anchor, amount of distance with closest gt box
        anchor_to_gt_min = anchor_by_gt_dist[np.arange(num_inside),
                                                anchor_to_gt_argmin]  # [N_anchor]
        # Map from gt box to an anchor that has minimum distance
        gt_to_anchor_argmin = anchor_by_gt_dist.argmin(axis=0) # [N_gt]
        # For each gt box, amount of distance with closest anchor
        gt_to_anchor_min = anchor_by_gt_dist[
            gt_to_anchor_argmin,
            np.arange(anchor_by_gt_dist.shape[1])] # [N_gt]
        # must remove gt which doesn't match any anchor.
        empty_gt_mask = gt_to_anchor_min >= unmatched_threshold[0]
        gt_to_anchor_min[empty_gt_mask] = -1
        # Find all anchors that share the min distance
        # (this includes many ties)
        anchors_with_min_dist = np.where(
            anchor_by_gt_dist == gt_to_anchor_min)[0]
        # Fg label: for each gt use anchors with minimum distance
        # (including ties)
        gt_inds_force = anchor_to_gt_argmin[anchors_with_min_dist]
        labels[anchors_with_min_dist] = gt_classes[gt_inds_force]
        gt_ids[anchors_with_min_dist] = gt_inds_force
        # Fg label: below threshold distance
        pos_inds = anchor_to_gt_min <= matched_threshold
        gt_inds = anchor_to_gt_argmin[pos_inds]
        labels[pos_inds] = gt_classes[gt_inds]
        gt_ids[pos_inds] = gt_inds
        bg_inds = np.where(anchor_to_gt_min >= unmatched_threshold)[0]
    else:
        # labels[:] = 0
    ...
    fg_max_overlap = None
    if len(gt_boxes) > 0 and anchors.shape[0] > 0:
        fg_max_overlap = anchor_to_gt_min[fg_inds]
    ...
    if len(gt_boxes) > 0 and anchors.shape[0] > 0:
        ...
        bbox_targets[fg_inds, :] = box_encoding_fn(
            gt_boxes[anchor_to_gt_argmin[fg_inds], :], anchors[fg_inds, :])
