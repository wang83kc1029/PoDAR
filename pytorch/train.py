...
def train(config_path,
    ...
    best_3d = 0
    best_step = 0
    all_3d = np.zeros((32, 1+len(class_names)*3))
    v_epoch = 0
    try:
        for _ in range(total_loop):
            ...
            for i, class_name in enumerate(class_names):
                ...
                all_3d[v_epoch, 0] = net.get_global_step()
                all_3d[v_epoch, (i*3+1):(i*3+4)] = mAP3d[i,:,0]
            v_epoch += 1
            np.savetxt(model_dir /'3d_all.txt', all_3d, fmt='%.2f')
            ...
            print(result, file=logf)
            print(result)
            if np.mean(mAP3d[:,:,0])>best_3d:
                best_3d = np.mean(mAP3d[:,:,0])
                best_step = net.get_global_step()
            print('Best 3DmAP at step: %d with mean: %.2f'%(best_step, best_3d), file=logf)
            print('Best 3DmAP at step: %d with mean: %.2f'%(best_step, best_3d))                
            if pickle_result:
