...
def polar_box_encode(boxes, anchors, encode_angle_to_vector=False, smooth_dim=False):
    """box encode for VoxelNet in polar coordinate
    Args:
    ...
    rada = torch.sqrt(xa**2 + ya**2)
    radg = torch.sqrt(xg**2 + yg**2)

    phia = torch.atan2(ya, xa)
    ...
    if encode_angle_to_vector:
        ...
        rtx = rgx - rax
        rty = rgy - ray
        return torch.cat([radt, phit, zt, wt, lt, ht, rtx, rty], dim=-1)
    else:
        rt = rg - ra
        return torch.cat([radt, phit, zt, wt, lt, ht, rt], dim=-1)
...
def polar_box_decode(box_encodings, anchors, encode_angle_to_vector=False, smooth_dim=False):
    ...
    xa, ya, za, wa, la, ha, ra = torch.split(anchors, 1, dim=-1)
    if encode_angle_to_vector:
        radt, phit, zt, wt, lt, ht, rtx, rty = torch.split(
            box_encodings, 1, dim=-1)
    else:
        radt, phit, zt, wt, lt, ht, rt = torch.split(box_encodings, 1, dim=-1)
    ...
    xg = radg * torch.cos(phig)
    yg = radg * torch.sin(phig)
