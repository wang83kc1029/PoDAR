...
class VoxelNet(nn.Module):
    def __init__(self,
        ...
        if not self._use_bev:
            ...
            if vfe_class_name in vfe_class_dict:
    ...
    def forward(self, example):
        ...
        t = time.time()
        if self._use_bev:
            preds_dict = self.rpn(example["bev_map"])
            v_dtype = example["bev_map"].dtype
        else:
            voxels = example["voxels"]
            v_dtype = voxels.dtype
