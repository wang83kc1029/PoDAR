...
class PillarFeatureNet(nn.Module):
    ...
    def forward(self, features, num_voxels, coors):
        ...
        ct_r = coors[:, 3].to(dtype).unsqueeze(1) * self.vx + self.x_offset
        ct_phi = coors[:, 2].to(dtype).unsqueeze(1) * self.vy + self.y_offset
        ...
        f_center[:, :, 0] = features[:, :, 0] - ct_r 
        f_center[:, :, 1] = (features[:, :, 1] - ct_phi) * ct_r

        # Combine together feature decorations
        features_ls = [features[:, :, :1], features[:, :, 2:], f_cluster, f_center]
